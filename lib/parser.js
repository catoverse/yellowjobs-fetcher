...
const parseJobType = (text) => {
  ...
  keywordMatch(text, (word) => {
    if (
      ...
      ["freshers", "zeroexperience", "noexperience", "fresher"].includes(word)
    ) {
      type = "internship"; //change this to freshers if we want freshers tweets classified seperately.
    } else {
...
const parseAdtFilters = (text) => {
  let type = [];

  keywordMatch(text, (word) => {
    if (["web3", "blockchain", "ethereum", "solidity", "nft"].includes(word)) {
      type.push("web3");
    } else if (["impact", "volunteer", "ngo"].includes(word)) {
      type.push("impact");
    }
  });
